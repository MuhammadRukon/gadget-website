...
export default function CheckoutPage() {
  ...
  // Recompute the quote whenever inputs that affect price change.
  useEffect(() => {
    if (!addressId) {
      setQuote(null);
      return;
    ...
    apiFetch<CheckoutQuote>('/api/checkout/quote', {
      ...
      .catch((err) => {
        if (cancelled) return;
        const message =
          err instanceof ApiClientError ? err.message : 'Could not calculate totals';
        toast.error(message);
        setQuote(null);
  ...
  const cartHasIssues = useMemo(() => {
    return !!cart.data?.lines.some((l) => !l.isActive || l.quantity > l.stock);
  }, [cart.data]);
  ...
  async function placeOrder() {
    ...
    try {
      ...
      if (res.redirectUrl) {
        // Hand off to the payment gateway. We use a hard navigation so
        // the gateway can post back via session cookies / referrer.
  ...
  if (status !== 'authenticated' || cart.isLoading || addresses.isLoading) {
    return (
