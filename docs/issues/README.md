...
| --- | --- | --- | --- |
| 1 | Payment success callbacks forgeable via `sandbox_` prefix | **CRITICAL** | security §1 |
| 2 | No amount verification on gateway callbacks | HIGH | security §2 |
| 3 | Stock oversell race in checkout | HIGH | correctness §1 |
| 4 | Coupon usage-limit race | HIGH | correctness §2 |
| 5 | In-memory rate limiting useless on Vercel serverless | HIGH | security §3 |
| 6 | Admin payment verify defaults to SUCCEEDED on empty body | MEDIUM | security §4 |
| 7 | Stock leaks on abandoned online payments (no restock/expiry) | MEDIUM | correctness §3 |
| 8 | Order status transitions unvalidated (any → any) | MEDIUM | correctness §4 |
...
| 12 | Default admin credentials in seed script | MEDIUM | security §5 |
| 13 | `allowDangerousEmailAccountLinking` enabled | MEDIUM | security §6 |
| 14 | JWT role staleness; middleware checks cookie presence only | MEDIUM | security §7 |
