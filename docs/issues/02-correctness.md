...
## ~~1. HIGH — Stock oversell race in checkout~~ (✅ completed)
...
## ~~2. HIGH — Coupon usage-limit race~~ (✅ completed)
...
## ~~3. MEDIUM — Stock leaks on abandoned online payments~~ (⚠️ partially completed — restock on FAILED/CANCELLED callbacks done; expiry sweep for stale PENDING orders still deferred, needs cron)
...
## ~~4. MEDIUM — Order status transitions unvalidated~~ (✅ completed — transition map + admin-cancel restock)
...
## ~~6. MEDIUM — Coupon validation inside checkout tx doesn't use the tx client~~ (✅ completed)
...
## ~~7. MEDIUM — Payment kickoff failure orphans the order~~ (✅ completed — order cancelled + restocked on kickoff failure)
...
## ~~11. LOW — Env var naming drift~~ (✅ completed — `NEXT_PUBLIC_APP_URL` standardized + zod-validated `src/env.ts`)
...
## ~~12. LOW — `sitemap.ts` conflicting route config~~ (✅ completed)
